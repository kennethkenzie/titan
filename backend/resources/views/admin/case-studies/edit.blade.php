...
@section('content')
    ...
    <div class="max-w-3xl">
        <form method="POST" action="{{ route('admin.case-studies.update', $caseStudy) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf
            @method('PUT')
        ...
        <div class="mt-6 flex justify-end border-t border-white/[0.06] pt-5">
            <form method="POST" action="{{ route('admin.case-studies.destroy', $caseStudy) }}"
                  onsubmit="return confirm('Delete this case study? This cannot be undone.')">
                @csrf
                @method('DELETE')
